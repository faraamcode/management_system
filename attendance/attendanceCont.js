...
exports.insertNewAttendance = async (req, res, next) => {
  ...
  const isAvailable = await Query.fetchByMultiple(
    table,
    checkfields,
    checkfieldvalue
  );
...
exports.DeleteAttendance = async (req, res, next) => {
  const fields = await Query.turnUpdateArrayWithAND([
    "admission_no",
    "term",
    "session",
  ]);
...
exports.UpdateAttendance = async (req, res, next) => {
  ...
  const updatefield = ["time_school_open", "present_time", "absent_time"];
  const clausefield = ["admission_no", "term", "session"];
  const fieldvalues = [
    req.body.time_school_open,
    req.body.present_time,
    req.body.absent_time,
  ...
  const result = await Query.UpdateWithMultiple(
    table,
    updatefield,
    clausefield,
    fieldvalues
  ...
  if (result === 1) {
    res.status(202).json({
      message: `${req.body.admission_no} attendance successfully updated`,
    });
  } else {
    res.status(400).json({
      message: "not updated",
